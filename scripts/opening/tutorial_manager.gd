...
class TutorialStep:
	...
	var highlight_nodes: Array = []  # Node paths to highlight (Array of Strings)
...
var tutorial_steps: Array = []  # Array of TutorialStep
...
Dragons must be well-rested to explore safely."""
	...
	step.custom_check = func() -> bool:
		...
		if not factory_manager or not factory_manager.factory:
			return false
		if factory_manager.factory.active_dragons.size() < 2:
			return false
		return true
...
func setup_action_listener(action: String) -> void:
	...
	match action:
		"dragon_created":
			if factory_manager and factory_manager.factory:
				if not factory_manager.factory.dragon_created.is_connected(_on_dragon_created):
					factory_manager.factory.dragon_created.connect(_on_dragon_created)
...
func apply_highlights(node_paths: Array) -> void:
	"""Highlight specific UI elements (expects Array of Strings)"""
