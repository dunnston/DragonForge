...
func advance_text(char_count: int) -> void:
	...
	for i in range(char_count):
		...
		# Check for thunder trigger phrase (when "They will come" appears)
		if not thunder_played and "They will come" in letter_text.text:
			play_thunder()
			thunder_played = true
...
func initialize_starting_inventory() -> void:
	...
	if TreasureVault and TreasureVault.instance:
		...
		print("[OpeningLetter] Added 30 starting gold")
	else:
